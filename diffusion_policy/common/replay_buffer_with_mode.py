...
class ReplayBufferWithMode:
    ...
    def __init__(self, 
        ...
        assert('mode_ends' in root['meta'])
        for key, value in root['data'].items():
            assert(
                value.shape[0]
                == root['meta']['episode_ends'][-1]
                == root['meta']['mode_ends'][-1]
            )
        self.root = root
    ...
    def create_empty_zarr(cls, storage=None, root=None):
        ...
        if 'mode_ends' not in meta:
            mode_ends = meta.zeros('mode_ends', shape=(0,), dtype=np.int64,
                compressor=None, overwrite=False)
    ...
    def create_empty_numpy(cls):
        root = {
            ...
            'meta': {
                ...
                'mode_ends': np.zeros((0,), dtype=np.int64)
            }
        }
        return cls(root=root)
    ...
    @property
    def mode_ends(self):
        return self.meta['mode_ends']
    
    @property
    def subtraj_ends(self):
        return self.mode_ends
    ...
    @property
    def subtraj_lengths(self):
        subtraj_ends = self.subtraj_ends.copy()
        subtraj_ends = np.insert(subtraj_ends, 0, 0)
        return np.diff(subtraj_ends)
    ...
    def add_episode(self, 
        ...
        # append to mode_ends
        # turn dense mode seq to sparse mode_ends
        mode_ends = self.mode_ends
        this_mode = data["mode"].copy()  # (T,)
        this_mode_ends = np.nonzero(
            np.diff(
                np.append(this_mode, [-1])  # append -1 to consider the last step
            )  # get the last sub step of each mode
        )[0] + 1  # +1 to get mode_ends j in [i, j)
        
        if is_zarr:
            mode_ends.resize(mode_ends.shape[0] + len(this_mode_ends))
        else:
            mode_ends.resize(mode_ends.shape[0] + len(this_mode_ends), refcheck=False)
        mode_ends[-this_mode_ends.shape[0]:] = curr_len + this_mode_ends

        # rechunk
        if is_zarr:
            if mode_ends.chunks[0] < mode_ends.shape[0]:
                rechunk_recompress_array(self.meta, 'mode_ends', 
                    chunk_length=int(mode_ends.shape[0] * 1.5))
    ...
    def drop_episode(self):
        ...
        mode_ends = self.mode_ends[:].copy()
        mode_ends_except_last_epi = mode_ends[mode_ends <= start_idx]
        if is_zarr:
            self.mode_ends.resize(len(mode_ends_except_last_epi))
        else:
            self.mode_ends.resize(len(mode_ends_except_last_epi), refcheck=False)
